...
public class BanCommand extends Command {
    ...
    protected void execute(CommandEvent e) {
        if(e.getArgs().length() == 0){
            EmbedBuilder embed = EmbedManager.getInstance().createEmbed("**Ban**", Color.BLUE, e.getMessage().getGuild().getName() + " ♢ " + e.getJDA().getSelfUser().getName(), "<@" + e.getMessage().getAuthor().getId() + "> You need to mention a name!");
            ...
            if(args[0].contains("@")) {
                e.getJDA().retrieveUserById(args[0].substring(3, args[0].length() - 1)).queue(user -> {
                    if(user != null){
                        BanService.getInstance().permBan(user);
                        String reason = "";
                        if(args.length > 1){
                        ...
                        EmbedBuilder embed = EmbedManager.getInstance().createEmbed("**Ban**", Color.BLUE, e.getMessage().getGuild().getName() + " ♢ " + e.getJDA().getSelfUser().getName(), "Succesfully banned: <@" + user.getId() + ">");
                        e.getMessage().getChannel().sendMessage(embed.build()).queue();
                    }else{
